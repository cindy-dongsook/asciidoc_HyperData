...
[JOB] > [Service Job] 메뉴를 선택하면 Service Job을 조회하거나 생성, 수정, 삭제할 수 있다. [생성] 버튼을 클릭하면 Service Job을 생성할 수 있고, 생성 후 초기상태는 Draft로 나타난다. Draft란 작업 수행 전의 Job을 뜻한다. 목록의 '동작' 항목에서 image:resources/icon_edit.png[width=20] 아이콘을 클릭하면 Service Job를 수정할 수 있다. 목록에서 Service Job을 선택한 후 [삭제] 버튼을 클릭하거나 '동작' 항목에 image:resources/icon_delete.png[width=20] 아이콘을 클릭하면 Service Job을 삭제할 수 있다. '동작' 항목에서 image:resources/icon_export.png[width=20] 아이콘을 클릭하면 Service Job을 다른 프로젝트에 공유할 수 있다.

**[스케줄 매니저]**버튼을 클릭하면 Service Job을 실행할 예약 시간 및 주기를 설정할 수 있다. 자세한 사용법은 <<sect_servicejob_schedule, Service Job 스케줄러 매니저>>를 참고한다.

목록 위에 '검색' 입력항목에 서비스 이름을 입력하고 <Enter> 키를 누르거나 image:resources/icon_search.png[width=22] 아이콘을 클릭하면 해당 이름의 서비스를 조회할 수 있다. 목록 아래에 조회된 건수와 페이징 정보, 조회할 건수를 선택할 수 있다.

목록에서 Service Job을 선택하면 Service Flow와 Service Job에 대한 내용을 화면 아래 영역에 조회되고, 'JOB 이름' 항목을 클릭하면 <<figure_servicejob_detail, Service Flow 생성화면>>으로 이동해서 Flow를 생성하거나 수정할 수 있다. 자세한 내용은 <<sect_servicejob_detail,Service Flow 생성 및 수정>>을 참고한다.

image::resources/figure_servicejob.png[title="Service Job 화면", width=750]
...
<<figure_servicejob, Service Job 화면>>의 목록에 '동작' 항목에서 image:resources/icon_edit.png[width=20] 아이콘을 클릭하면 Service Job를 수정할 수 있다. 각 항목을 수정한 후 [확인] 버튼을 클릭한다. Service Job을 수정하는 경우 프로젝트는 변경할 수 없다.
...
<<figure_servicejob, Service Job 화면>>의 목록에서 Service Job을 선택한 후 [삭제] 버튼을 클릭하거나 '동작' 항목에 image:resources/icon_delete.png[width=20] 아이콘을 클릭하면 Service Job을 삭제할 수 있다. 목록에서 삭제할 정보를 확인한 후 [삭제] 버튼을 클릭한다.
...
<<figure_servicejob, Service Job 화면>>의 목록에서 Data Job을 선택한 후 '동작' 항목에 image:resources/icon_export.png[width=20] 아이콘을 클릭하면 Service Job을 다른 프로젝트에 공유할 수 있다. 프로젝트 목록 위에 '검색' 입력항목에 검색할 프로젝트를 입력하고 image:resources/icon_search.png[width=22] 아이콘을 클릭하면 공유할 프로젝트를 조회할 수 있다. 공유할 Job 이름을 입력하고 프로젝트를 선택한 후 [적용] 버튼을 클릭한다.
...
<<figure_servicejob, Service Job 화면>>의 목록에서 'JOB 이름' 항목을 클릭하면 Service Job의 Flow 정보를 조회하거나 생성할 수 있다.

image::resources/figure_servicejob_detail.png[title="Service Flow 생성화면", width=750]

다음은 Service Flow 생성화면의 각 영역에 대한 설명이다. 기본적인 사용법은 <<figure_datajob_detail, Data Job 상세화면>>과 동일하므로 자세한 내용은 <<sect_datajob_detail, @@>>을 참고한다.
...
|=====================================================================================================================
|버튼															|설명
|image:resources/icon_undo.png[width=16](Undo) / image:resources/icon_redo.png[width=16](Redo)|
|image:resources/icon_save.png[width=16](Save)|
|image:resources/icon_run.png[width=22](Run)|
|image:resources/icon_stop.png[width=16](Stop)|
|image:resources/icon_zoomout.png[width=16](Zoom In) / image:resources/icon_zoomin.png[width=16](Zoom Out)|
|image:resources/icon_panmode.png[width=16](Pan Mode)|
|image:resources/icon_fit.png[width=16](Fit)|
|image:resources/icon_acturalsize.png[width=20](Actual Size)|
|image:resources/icon_minimap.png[width=16](Mini Map)|
|=====================================================================================================================
...
+
[#figure_servicejob_automl_inference_dataobject]
...
* Inference
+
'Job Type' 항목을 'Inference'로 선택하면 PredefinedAI에서 생성한 모델에 대해 추론을 수행할 수 있다. 노드를 클릭하면 PredefinedAI 실험 정보가 조회된다. Inference Setting에 Inference 시 필요한 설정을 선택하고 Service Flow를 수행한다. Inference Setting에 대한 자세한 방법은 <<figure_servicejob_automl_inference_dataobject,"Inference Setting">>을 참고한다.
+
...
+
[#figure_diverging_gateway_type_trigger]
...
+
<<figure_diverging_gateway_type_trigger, "Service Flow - 분기 처리">>에서 [+트리거] 버튼을 클릭해서 Data Job의 서비스에 대한 트리거를 설정할 수 있다. 각 항목을 설정한 후 [등록] 버튼을 클릭한다.
+
image::resources/figure_diverging_gateway_add_trigger.png[title="Service Flow - 트리거 설정", width=300]
+
다음은 각 항목에 대한 설명이다.
+
[width="100%",cols="3,7",options="header"]
...
////
 ...
 ---------------------
////
[#sect_servicejob_schedule]
==== Service Job 스케줄러 매니저

 
<<figure_servicejob, Service Job 화면>>에서 *[스케줄 매니저]* 버튼을 클릭하면 Service Job을 실행할 예약 시간 및 주기를 설정할 수 있다. [생성] 버튼을 클릭하면 스케줄을 생성할 수 있다. 목록에서 스케줄을 선택한 후 [수정], [삭제] 버튼을 클릭하면 스케줄을 수정하거나 삭제할 수 있다.

목록 위에 '검색' 입력항목에 스케줄 이름을 입력하고 <Enter> 키를 누르거나 image:resources/icon_search.png[width=22] 아이콘을 클릭하면 해당 이름의 스케줄을 조회할 수 있다. 목록 아래에 조회된 건수와 페이징 정보, 조회할 건수를 선택할 수 있다. 목록을 선택하면 스케줄의 Service Job Flow가 아래 상세 영역에 조회된다. 목록의 '동작/정지' 항목의 토글 버튼을 선택해서 스케줄을 동작시킬 수 있다.
...
+
image::resources/figure_servicejobschedule_dates_create.png[width=400]
...
+
image::resources/figure_servicejobschedule_cron_create.png[width=400]
