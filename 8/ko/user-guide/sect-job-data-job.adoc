...
|====================================
|버튼	|설명
|image:resources/icon_undo.png[title="",width=16](Undo) / image:resources/icon_redo.png[title="",width=16](Redo)	|되돌리기/ 앞으로 가기 버튼이다. Data Flow를 생성하는 과정에서 사용
할 수 있다.
|image:resources/icon_save.png[title="",width=16](Save)	|Data Flow를 저장한다. 저장하지 않은 경우 현재까지 작성한 Flow는
보존되지 않는다.
|image:resources/icon_run.png[title="",width=22](Run)		|Data Flow를 수행한다. 수행 결과는 [Output] 탭에서 조회할 수 있다.
|image:resources/icon_stop.png[title="",width=16](Stop)	|	Data Flow를 수행을 중지한다. (현재 지원하지 않음)
|image:resources/icon_valiables.png[title="",width=16](Show Variable)|Data Flow를 수행하는 경우 각 노드를 설정할 때 동일값 반복입력이 필
요한 경우 변수를 사용할 수 있다. Variable 화면([그림 5.8])에서 변수를
선택할 수 있다.
|image:resources/icon_sql.png[title="",width=16](Show SQL View)	|Data Flow를 작동한 이후 성공적으로 작동한 Job에 대한 SQL문을 볼
수 있다.
|image:resources/icon_zoomout.png[title="",width=16](Zoom In) / image:resources/icon_zoomin.png[title="",width=16](Zoom Out) |Data Flow 작업영역을 확대, 축소한다.
|image:resources/icon_panmode.png[title="",width=16](Pan Mode) |Pan Mode를 선택하면 마우스 클릭만으로 화면을 움직일 수 있다. 해당
아이콘을 한번더 눌리면 Pan Mode를 해제할 수 있다.
|image:resources/icon_fit.png[title="",width=16](Fit)	|작업공간에 만든 Data Flow 영역을 가르킨다.
|image:resources/icon_acturalsize.png[title="",width=20](Actual Size)	|처음에 Data Flow을 생성했던 size로 볼 수 있다.
|image:resources/icon_minimap.png[title="",width=16](Mini Map)	|해당 아이콘을 클릭하면 미니맵이 사라진다. 한번더 클릭하면 미니맵을
볼 수 있다.
|====================================
+
...
+
image::resources/figure_data_manipulation_setoperators_key.png[title="",width=500]
